...
class Detector:
    ...
    def Prediction_2(self):

        global pred_list_3, xmax1, output, crop_box_areas_2, pred_list_2, new_coors2
        for detection in self.pred_list:
            ...
            # Inference
            pred = self.model(im, augment=self.augment, visualize=self.visualize)
            pred = non_max_suppression(pred, self.conf_thres, self.iou_thres, 0, self.agnostic_nms,
            ...
            for i, det in enumerate(pred):  # per image
                ...
                if len(det):
                    ...
                    for *xyxy, conf, cls in reversed(det):
                        ...
                        pred_list_2.append([float(i) for i in np.array(crop_box_areas_2)])

        self.pred_list_3 = []

        for det in pred_list_2:
            output2 = list(det[:4])
            output2 = [int(x) for x in output2]
            new_coors1 = [xmax1 + output2[0]-2,
                         output[1] + output2[1]-2,
                         (output2[2] - output2[0]) + xmax1 + output2[0]+1,
                         (output2[3] - output2[1]) + output[1] + output2[1]+1]
            self.pred_list_3.append([int(i) for i in np.array(new_coors1)])

            image = cv2.rectangle(self.im0, tuple(new_coors1[:2]), tuple(new_coors1[2:]), (0, 0, 255), 1)
            cv2.imwrite("output.jpg", image)

        return self.pred_list_3
